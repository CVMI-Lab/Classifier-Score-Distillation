...
@threestudio.register("csd-multiview-diffusion-guidance")
class MultiviewDiffusionGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        self.alphas_cumprod: Float[Tensor, "T"] = self.model.alphas_cumprod.to(
            self.device
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.cfg.grad_normalize:
            normalized_grad = grad / (grad.std(dim=(-2, -1), keepdim=False).unsqueeze(2).unsqueeze(3) + 1e-8)
            grad = self.cfg.grad_normalize_scale * normalized_grad + (1-self.cfg.grad_normalize_scale) * grad
